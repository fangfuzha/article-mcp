...
**对齐目标：** 新增 `SearchCache`（`src/middleware/search_cache.ts`），支持 SHA256 键生成、文件存储、TTL 过期、命中/未命中统计。
